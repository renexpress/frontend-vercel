...
function Products() {
  ...
  const filteredProducts = products.filter(product => {
    ...
    if (searchQuery.trim()) {
      ...
      if (searchFilters.size && product.variants) {
        const hasSizeMatch = product.variants.some(v =>
          v.variant_attributes?.some(attr =>
            attr.attribute_name === 'Размер' && attr.value?.toLowerCase().includes(query)
          )
        );
        if (hasSizeMatch) matches = true;
      }
      if (searchFilters.color && product.variants) {
        const hasColorMatch = product.variants.some(v =>
          v.variant_attributes?.some(attr =>
            attr.attribute_name === 'Цвет' && attr.value?.toLowerCase().includes(query)
          )
        );
        if (hasColorMatch) matches = true;
      }
